...
import { useCreateAsset, useUpdateAsset } from '@/hooks/useSupabaseData'
...
export const AssetForm: React.FC<AssetFormProps> = ({ open, onOpenChange, asset }) => {
  ...
  const onSubmit = async (data: AssetFormData) => {
    try {
      ...
      if (asset) {
        await updateAsset.mutateAsync({ id: asset.id, ...safeData })
        toast({
          title: 'Success',
          description: 'Asset updated successfully',
        })
      } else {
        await createAsset.mutateAsync(safeData)
        toast({
          title: 'Success',
